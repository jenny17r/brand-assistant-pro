...
export function CsvUpload() {
    ...
    return (<div onDragOver={(e) => {
        ...
        }} className={cn("rounded-xl border-2 border-dashed p-8 text-center transition-colors", drag ? "border-foreground bg-muted/60" : "border-border bg-muted/20")}>
      <input ref={inputRef} type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={(e) => e.target.files?.[0] && setFile(e.target.files[0])}/>
