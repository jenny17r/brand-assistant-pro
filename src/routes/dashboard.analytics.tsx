    ...
    <div className="p-6 lg:p-10 space-y-8 max-w-7xl mx-auto">
      ...
      <section className="rounded-xl border border-border bg-card shadow-sm">
        <div className="p-5 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Ranked by revenue contribution.</p>
        </div>
        <Table>
          <TableHeader>
            <TableRow>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border bg-card shadow-sm p-5 space-y-3">
          ...
          <Alert tone="warning" icon={AlertTriangle} title="High revenue concentration" body="Laptop contributes 68% of total revenue." />
          <Alert tone="info" icon={Info} title="Low contribution" body="Keyboard contributes only 4% of revenue." />
          <Alert tone="info" icon={Info} title="Low contribution" body="Mouse contributes only 3% of revenue." />
        </div>
      ...
      <section className="rounded-xl border border-border bg-card shadow-sm p-5 space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Upload a CSV or Excel file to refresh your dashboard.</p>
        </div>
        <CsvUpload />
      </section>
    </div>
...
function Kpi({ label, value, icon: Icon, delta, up }: { label: string; value: string; icon: React.ComponentType<{ className?: string }>; delta: string; up?: boolean }) {
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm p-5">
      ...
      <div className={"flex items-center gap-1 text-xs mt-1.5 " + (up ? "text-emerald-600" : "text-muted-foreground")}>
...
function Alert({ tone, icon: Icon, title, body }: { tone: "warning" | "info"; icon: React.ComponentType<{ className?: string }>; title: string; body: string }) {
  const toneClasses = tone === "warning" ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-sky-50 text-sky-700 border-sky-200";
  return (
