...
import { AlertTriangle, ArrowDown, ArrowUp, ArrowUpRight, CheckCircle2, Crown, Info, Lightbulb, Percent, PiggyBank, Receipt, Sparkles, TrendingDown, TrendingUp, Wallet, } from "lucide-react";
...
export const Route = createFileRoute("/dashboard/analytics")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Analyze sales performance, profit metrics, alerts and recommendations.",
            },
        ],
    }),
    ...
    return (<div className="p-6 lg:p-10 space-y-8 max-w-7xl mx-auto">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Revenue by product" subtitle="Gross revenue per SKU">
          <BarsChart data={PRODUCTS.map((p) => ({ name: p.name, value: p.revenue }))}/>
        </ChartCard>
        <ChartCard title="Profit by product" subtitle="Net profit per SKU">
          <BarsChart data={PRODUCTS.map((p) => ({ name: p.name, value: p.profit }))}/>
        </ChartCard>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border bg-card shadow-sm p-5 space-y-3">
          ...
          <AlertCard severity="warning" title="High revenue concentration" body="Laptop contributes 68% of total revenue."/>
          <AlertCard severity="info" title="Low contribution" body="Keyboard contributes only 4% of revenue."/>
          <AlertCard severity="info" title="Low contribution" body="Mouse contributes only 3% of revenue."/>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <InsightCard icon={Crown} label="Most profitable product" value="Laptop" sub="₹100,000 net profit" tone="emerald"/>
          <InsightCard icon={TrendingDown} label="Least profitable product" value="Keyboard" sub="₹12,000 net profit" tone="amber"/>
          <InsightCard icon={Percent} label="Revenue concentration" value="68%" sub="Top SKU share of revenue" tone="sky"/>
          <InsightCard icon={CheckCircle2} label="Business health" value="Healthy" sub="Margin above 20%" tone="emerald"/>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <ForecastCard label="Next month revenue forecast" value={fmt(780000)} sub="Projected gross revenue" icon={Wallet}/>
          <ForecastCard label="Expected growth" value="+6%" sub="vs. current period" icon={ArrowUpRight} highlight/>
          <ForecastCard label="Top predicted product" value="Laptop" sub="Forecast leader by revenue" icon={Crown}/>
        </div>
...
/* -------------------- Reusable components -------------------- */
function Kpi({ label, value, icon: Icon, delta, up, status, }) {
    return (<div className="rounded-xl border border-border bg-card shadow-sm p-5">
      <div className="flex items-center justify-between">
      ...
      {delta ? (<div className={cn("flex items-center gap-1 text-xs mt-1.5", up ? "text-emerald-600" : "text-muted-foreground")}>
          {up ? <TrendingUp className="h-3.5 w-3.5"/> : <TrendingDown className="h-3.5 w-3.5"/>}
          <span>{delta} vs last period</span>
        </div>) : status ? (<div className="flex items-center gap-1 text-xs mt-1.5 text-emerald-600">
...
function BarsChart({ data }) {
    ...
    return (<ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
        ...
        <Tooltip cursor={{ fill: "rgba(15,23,42,0.04)" }} contentStyle={{
            borderRadius: 8,
...
function ProductsTable() {
    ...
    const SortHeader = ({ label, k, align = "left", }) => (<TableHead className={align === "right" ? "text-right" : ""}>
      <button type="button" onClick={() => toggle(k)} className={cn("inline-flex items-center gap-1 hover:text-foreground transition-colors", align === "right" && "ml-auto")}>
        {label}
        {sort.key === k ? (sort.dir === "asc" ? (<ArrowUp className="h-3 w-3"/>) : (<ArrowDown className="h-3 w-3"/>)) : null}
      </button>
    </TableHead>);
    return (<section className="rounded-xl border border-border bg-card shadow-sm">
      ...
      <Table>
        ...
        <TableBody>
          {rows.map((p) => (<TableRow key={p.name} className="hover:bg-muted/40 transition-colors">
              ...
              <TableCell className="text-right">
                <div className="flex items-center justify-end gap-2">
                  <div className="w-24 h-1.5 rounded-full bg-muted overflow-hidden">
                    <div className="h-full bg-foreground" style={{ width: `${p.contribution}%` }}/>
                  </div>
...
function StatusBadge({ status }) {
    ...
    return (<span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium", map[status])}>
      {status}
    </span>);
}
function AlertCard({ severity, title, body, }) {
    const tone = severity === "warning"
        ? "bg-amber-50 text-amber-700 border-amber-200"
        : "bg-sky-50 text-sky-700 border-sky-200";
    const Icon = severity === "warning" ? AlertTriangle : Info;
    return (<div className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/30">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-[10px] uppercase tracking-wide rounded px-1.5 py-0.5 border", tone)}>
            {severity}
          </span>
        </div>
...
function RecommendationCard({ body }) {
    return (<div className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/30">
...
function ForecastCard({ label, value, sub, icon: Icon, highlight, }) {
    return (<div className={cn("rounded-xl border p-5 shadow-sm", highlight
            ? "border-emerald-200 bg-emerald-50/40"
            : "border-border bg-card")}>
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">{label}</p>
        <div className={cn("h-8 w-8 rounded-lg grid place-items-center border", highlight
