...
import { Palette, PenSquare, Megaphone, BarChart3, Sparkles, TrendingUp, Lightbulb, Target, CalendarDays, CheckCircle2, Clock, Upload, FileText, } from "lucide-react";
...
const festivals = [
    { name: "Diwali", month: 11, day: 1, readiness: "ready" },
    { name: "Pongal", month: 1, day: 14, readiness: "draft" },
    { name: "Christmas", month: 12, day: 25, readiness: "not-started" },
];
function DashboardHub() {
    ...
    return (<div className="p-6 lg:p-10 space-y-10 max-w-7xl mx-auto">
      {/* Header */}
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((a) => (<div key={a.title} className="group rounded-xl border border-border bg-card shadow-sm p-5 flex flex-col hover:shadow-md transition-shadow">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-card shadow-sm p-5">
          ...
          <ol className="mt-4 relative border-l border-border ml-2 space-y-5">
            {activity.map((a, i) => (<li key={i} className="pl-5">
        ...
        <div className="lg:col-span-3 space-y-4">
          <SectionHeading title="Business insights" subtitle="AI-generated observations from your latest data." compact/>
      ...
      {/* Upcoming Festivals */}
      <section className="space-y-4">
        <SectionHeading title="Upcoming festivals" subtitle="Plan AI-generated campaigns ahead of key dates."/>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {festivals.map((f) => (<FestivalCard key={f.name} {...f}/>))}
        </div>
      </section>
    </div>);
}
function SectionHeading({ title, subtitle, compact, }) {
    return (<div>
...
function InsightCard({ icon: Icon, title, body, tone, }) {
    const toneMap = {
...
function FestivalCard({ name, month, day, readiness, }) {
    ...
    return (<div className="rounded-xl border border-border bg-card shadow-sm p-5 flex flex-col">
      ...
      <div className={"mt-4 inline-flex items-center gap-1.5 self-start text-xs px-2 py-1 rounded-md border " +
            r.classes}>
