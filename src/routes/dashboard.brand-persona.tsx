...
const DEFAULT_PERSONA: Persona = {
  ...
  description:
    "TechMart is a modern electronics retailer focused on providing affordable technology solutions to students and professionals.",
  ...
  audience:
    "Primary audience includes students, young professionals and technology enthusiasts aged 18–35.",
  voice: [
  ...
  summary:
    "This brand focuses on delivering reliable technology products while maintaining a professional and approachable identity.",
};
...
function BrandPersonaPage() {
  ...
  const [platforms, setPlatforms] = useState<Platform[]>(["Instagram", "WhatsApp"]);
  const [persona, setPersona] = useState<Persona | null>(DEFAULT_PERSONA);
...
function PersonaForm({
  ...
  return (
    <section className="rounded-xl border border-border bg-card shadow-sm p-5 space-y-5">
      ...
      <div className="space-y-4">
        ...
        <Field label="Brand tone">
          <Select value={form.brandTone} onValueChange={(v) => update("brandTone", v)}>
            ...
            <SelectContent>
              {["Professional", "Friendly", "Premium", "Playful", "Bold"].map((t) => (
...
function LoadingPanel({ step }: { step: number }) {
  return (
    <section className="rounded-xl border border-border bg-card shadow-sm p-8 text-center space-y-5">
      ...
      <ul className="max-w-sm mx-auto space-y-2 text-left">
        {LOADING_STEPS.map((s, i) => (
          <li
            key={s}
            className={cn(
              "flex items-center gap-2 text-sm rounded-md border border-border bg-muted/40 px-3 py-2",
              i < step && "opacity-70",
              i === step && "ring-1 ring-foreground/20",
            )}
          >
            {i < step ? (
    ...
    <div className="space-y-6">
      ...
      <SectionCard icon={Megaphone} title="Brand voice guidelines">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {persona.voice.map((v) => (
            <div key={v.title} className="rounded-lg border border-border bg-muted/30 p-4">
...
function PersonalityChips({ traits }: { traits: string[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {traits.map((t) => (
        <span
          key={t}
          className="text-xs rounded-full border border-border bg-muted/50 px-3 py-1 font-medium"
        >
          {t}
        </span>
      ))}
